...
export default function CaseStudyCard({ caseData }: CaseStudyCardProps) {
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-lg overflow-hidden border ${caseData.is_featured ? 'border-2 border-blue-500' : 'border-gray-200'}`}>
      {/* Featured Badge */}
      {caseData.is_featured && (
        <div className="bg-blue-600 text-white text-xs font-bold px-3 py-1 inline-block">
          FEATURED CASE
        </div>
      )}
      ...
      <div className="p-6">
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {caseData.deliverables.slice(0, 5).map((item, index) => (
              <span 
                key={index}
            ...
            {caseData.deliverables.length > 5 && (
              <span className="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-xs">
                +{caseData.deliverables.length - 5} more
              </span>
            )}
          </div>
        </div>
